...
# 在庫/予約系スナップショット。orders/performance とは別のスクレイピング flow で
# 取得されるため、まとめて落ちることがある (例: 2026-06-14 / 06-21 は在庫分析・倉庫
# 在庫・予約管理が同時欠損、orders/performance は成功)。点群スナップショットなので
# 過去日は復旧不可 → 早期検知して当日中に再取得するのが唯一の対策。
SNAPSHOT_TABLES = [
    ("stock_analysis",      "snapshot_date",    "在庫分析(在庫)"),
    ("inventory_snapshot",  "snapshot_date",    "倉庫在庫"),
    ("reservations",        "reservation_date", "予約管理"),
    # Sitateru も別サイト(direct.sitateru.com)の別フローで取得。2026-06-17 以降
    # 取得停止 → サイレントに古くなる (シーズン反映に影響) ため監視対象に追加。
    ("sitateru_item_master", "snapshot_date",   "シタテル(商品メタ)"),
]
...
def _table_daily_counts(bq: bigquery.Client, start: date, end: date,
                        table: str, datecol: str) -> dict[str, int]:
    """任意テーブルの各日 (datecol) の行数を返す (在庫/予約系スナップショット用)。"""
    q = f"""
      SELECT CAST({datecol} AS STRING) AS d, COUNT(*) AS n_rows
      FROM `{GCP_PROJECT_ID}.{BQ_DATASET_ANALYTICS}.{table}`
      WHERE {datecol} BETWEEN DATE(@s) AND DATE(@e)
      GROUP BY d
    """
    cfg = bigquery.QueryJobConfig(query_parameters=[
        bigquery.ScalarQueryParameter("s", "STRING", start.isoformat()),
...
def main() -> int:
    ...
    ap.add_argument("--skip-snapshot", action="store_true",
                    help="在庫/倉庫/予約スナップショットの欠損チェックを行わない")
    ...
    # ── (4) 在庫/倉庫/予約スナップショット欠損 ────────────────────────────
    # これらは別フローで取得されるため orders/performance が成功していても
    # まとめて欠損し得る。各テーブルの取得開始日以降で 0 行の日を検知する。
    snap_missing: dict[str, list[str]] = {}   # label -> missing dates
    if not args.skip_snapshot:
        for table, datecol, label in SNAPSHOT_TABLES:
            cnts = _table_daily_counts(bq, start, end, table, datecol)
            if not cnts:
                continue
            first = min(cnts)   # 取得開始日より前は誤検知しない
            miss = []
            d = start
            while d <= end:
                ds = d.isoformat()
                if ds >= first and cnts.get(ds, 0) == 0:
                    miss.append(ds)
                d += timedelta(days=1)
            if miss:
                snap_missing[label] = miss
            logger.info("[snapshot] %-12s (%s): from %s, missing_days=%d",
                        label, table, first, len(miss))

    # ── 集計・判定 ────────────────────────────────────────────────────────
    any_gap = bool(ord_missing or ord_low or perf_missing or perf_low
                   or shop_gaps or snap_missing)

    if not any_gap:
        logger.info("OK: no gaps detected (orders + performance + shops + snapshots).")
    ...
    for label, miss in snap_missing.items():
        logger.info("  SNAPSHOT %-12s : 欠損日 %s", label, miss)
    ...
    if snap_missing:
        slack_lines.append("*SNAPSHOT GAP* (在庫/倉庫/予約 — 点群のため当日中の再取得が必要): "
                           f"{snap_missing}")
    if ord_bad:
