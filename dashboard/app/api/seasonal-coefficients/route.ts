...
export async function GET() {
  try {
    // 画面は coefficient_raw（生の実数）を表示。シミュレーションは別途 coefficient（補正後）を使用。
    const [rows] = await bq().query({
      query: `SELECT gender, child_item_type, week_number, coefficient_raw, week_qty, coef_source,
    ...
    const map = new Map<string, { gender: string; child_item_type: string; weeks: (number | null)[]; total: number; from_parent: boolean }>();
    let updatedAt: string | null = null;
    for (const r of rows as any[]) {
      updatedAt = r.updated_at ?? updatedAt;
      const key = `${r.gender}|||${r.child_item_type}`;
      if (!map.has(key)) map.set(key, { gender: r.gender, child_item_type: r.child_item_type, weeks: Array(54).fill(null), total: 0, from_parent: false });
      const g = map.get(key)!;
      g.weeks[Number(r.week_number)] = Number(r.coefficient_raw);
      g.total += Number(r.week_qty) || 0;
      if (r.coef_source === 'parent') g.from_parent = true;
    }
